...
void       do_munmap(struct vma_struct *, uint64, uint64);
int        do_mmap_copy(pagetable_t, pagetable_t, uint64, uint64);
void       put_mmap(struct proc *, struct vma_struct *);
...
int do_no_page(struct vma_struct *vma, uint64 vaddr) {
  char *mem;
  struct proc *p = myproc();
...
void do_munmap(struct vma_struct *vma, uint64 vaddr, uint64 len) {
  uint64 a;
  pte_t *pte;
  struct proc *p = myproc();
...
void put_mmap(struct proc *p, struct vma_struct *vma) {
  struct vma_struct *slot;
